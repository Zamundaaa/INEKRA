...
public class ShadowMapMasterRenderer {
	...
	private static final int SHADOW_MAP_SIZE = 4096;// *2

	private static ShadowFrameBuffer shadowFbo;
	private static ShadowShader shader;
	private static ShadowChunkShader scs;
	...
	private static ShadowMapEntityRenderer entityRenderer;
	 ...
	 */
	public static void render(Map<TexturedModel, List<MWBE>> entities, List<ChunkEntity> ces, Light sun) {
		shadowBox.update();
		...
		scs.start();
		entityRenderer.renderChunks(ces);
		scs.stop();
	 ...
	 * FBOs depth attachment from last frame. The simple shader program is NOT
	 * started!
	 * 
	...
	/**
	 * Finish the shadow render pass. Unbinds the shadow FBO, so everything
	 * rendered after this point is rendered to the screen, rather than to the
	 * shadow FBO.
	 */
	...
	private static void updateLightViewMatrix(Vector3f direction, Vector3f center) {
		...
		lightViewMatrix.rotate(pitch, Vects.XP);
		float yaw = (float) Math.toDegrees(((float) Math.atan(direction.x / direction.z)));
		yaw = direction.z > 0 ? yaw - 180 : yaw;
		lightViewMatrix.rotate((float) -Math.toRadians(yaw), Vects.UP);
