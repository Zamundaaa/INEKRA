	...
	public static void fillSphere(int sx, int sy, int sz, int r, short ID) {
		...
		for (int x = -r; x <= r; x++) {
			for (int y = -r; y <= r; y++) {
				for (int z = -r; z <= r; z++) {
					if (x * x + y * y + z * z <= r2) {
						Intraface.setBlock(x + sx, y + sy, z + sz, ID);
					}
				}
			}
		}
...
//		EXPLOSION((int) position.x, (int) position.y, (int) position.z, r);
		...
		explosionHelper = new Thread("ExplosionHelper"){
			...
			public void run(){
				while(ThreadManager.running()){
					while(explosionQueue.isEmpty() && ThreadManager.running()){
						Meth.wartn(5);
					}
	...
	public static void replaceAllAncients(int x, int y, int z, short repID, int CAP) {
		...
		if (id != Block.AIR) {
			ChunkManager.dontDropItems();
			counter = 0;
			Intraface.setBlock(x, y, z, repID);
	...
	private static void replaceHelp(int x, int y, int z, short startID, short repID, int CAP) {
		...
		if (b == startID) {
			Intraface.setBlock(x + 1, y, z, repID);
			counter++;
		...
		if (b == startID) {
			Intraface.setBlock(x, y, z + 1, repID);
			counter++;
		...
		if (b == startID) {
			Intraface.setBlock(x, y + 1, z, repID);
			counter++;
